...
**Core idea:** Seed scenario in → Decompose into dimensions → Generate situations → Classify (new/duplicate/variant) → Expand edge cases → Log → Repeat. Every iteration explores one unexplored combination. Treat `/autoresearch:scenario` as an alias for this workflow.
...
If the scenario is missing or too vague, scan the repo or feature context first, then ask a compact set of direct plain-text questions before proceeding. Keep the interaction adaptive, but do not depend on any special question schema or UI-specific tool.
...
Ask the selected questions in one concise batch:
...
**IMPORTANT:** Batch all selected questions together. Never ask them one at a time; users need full context to make informed decisions together.
...
- If the user cancels setup → exit cleanly with message: "Scenario exploration cancelled. Run `/autoresearch:scenario` again when ready."
...
**STOP: Have you completed the Interactive Setup above?** If invoked without scenario/flags, you MUST complete the setup questions above BEFORE entering this phase.
