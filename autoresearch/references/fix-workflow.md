...
**Core idea:** Detect → Prioritize → Fix ONE thing → Verify → Keep/Revert → Repeat until zero errors. Treat `/autoresearch:fix` as an alias for this workflow.
...
If `--target`, `--guard`, or `--scope` is missing, first auto-detect failures, then ask direct plain-text questions before proceeding. Keep the setup concise and grounded in the scan results; no special UI tool is required.

Suggested prompts:
1. "What should I fix?"
2. "What command must always pass?"
3. "Which files can I modify?"
4. "Ready to fix now, or should I use an iteration limit?"

If the failure set is already explicit, skip the questions and proceed.
...
**STOP: Have you completed the Interactive Setup above?** If invoked without `--target`/`--guard`/`--scope` flags, you MUST complete the setup questions above BEFORE entering this phase.
