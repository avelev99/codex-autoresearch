...
Autonomous codebase documentation engine. Scout codebase structure, learn patterns and architecture, generate or update comprehensive documentation, then validate and iteratively improve until docs are accurate. Treat slash-command examples as legacy aliases only; Codex can invoke this workflow from natural-language requests or direct task framing.
...
If `--mode` or enough inline context is missing, ask the user directly in plain text before Phase 1. Keep the prompt short and structured, but do not depend on a special question tool or schema lookup.
...
**Cancel handling:** If the user cancels, exit cleanly with a short cancellation message.
...
/autoresearch:learn
  ...
  ├── Phase 4: Generate — Produce docs with a structured prompt
...
### Update Mode — Read Existing Docs

Default to local reading and summarization. Use subagents only when the user explicitly asked for delegation or parallel agent work.
...
4. **Strategy by count:**
- **0 files:** Ask the user directly whether to switch to init mode. If yes, restart as init. If cancel, stop.
   - **1-3 files:** Read directly in the main thread
   - **4-6 files:** Prefer local reading with concise per-file notes; use subagents only if explicitly authorized
   - **7+ files:** Consider subagents only if explicitly authorized; otherwise sample strategically and summarize to stay within context budget
5. **Context budget:** If total docs LOC >5000 → warn in the generation prompt about summarization
6. Distribute attention by LOC; give larger files dedicated summary space or delegated review only if explicitly authorized
7. **Per-file summary contract:**
   ```
   ...
   **Needs update:** areas likely stale or incomplete based on content
   ...
   Files: {list}"
   ```
8. Merge the summaries into the working context
...
- Instruct the documentation pass to focus regeneration effort on mapped docs, light-touch others
...
## Phase 4: Generate — Produce Documentation

Generate the docs in the main thread by default. Only delegate to a dedicated docs worker if the user explicitly asked for subagents or parallel agent work.
...
2. **Doc reading results** (from Phase 3 summaries — Update mode only)
...
LOOP:
  ...
  2. Re-run the documentation pass with:
