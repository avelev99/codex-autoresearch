...
Multi-persona swarm prediction that pre-analyzes code from multiple expert perspectives. Simulate 3-5 personas locally by default, then produce ranked findings and hypotheses. Use explicit subagents only when the user has asked for delegation or parallel agent work. Natural-language requests or explicit task text trigger this workflow; slash-command examples are legacy aliases only.
...
If scope, goal, or depth is missing, ask the user directly in plain text after a quick codebase scan. Keep the questions batched in a single message when possible, but do not depend on a special question tool or UI schema.
...
| 4 | `Chain` | "After analysis, chain to another workflow?" | If no chain target is provided | "Debug", "Security", "Fix", "Ship", "Scenario", "No chain — report only" |

**IMPORTANT:** Batch all selected questions into one concise prompt if you need clarification. Do not block on tool availability.
...
**STOP: Have you completed the interactive setup above?** If scope, goal, or depth is still missing, pause and ask the user before entering this phase.
...
- Validate chain target(s). Supports a single target or a comma-separated sequence such as `debug,scenario,fix`. Each target must be a known downstream workflow. Unknown targets should be rejected or normalized before execution. Multi-chain executes sequentially and feeds each stage's findings into the next via handoff data. `Iterations` applies only to predict, not the chained workflows.
