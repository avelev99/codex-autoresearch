...
**Output:** A severity-ranked security report with threat model, findings, mitigations, and iteration log. Treat `/autoresearch:security` as an alias for this workflow.
...
If `--diff`, scope, or focus is missing, scan the codebase first, then ask direct plain-text questions before proceeding. Use the scan to suggest defaults and keep the setup short; no special UI tool is required.

Suggested prompts:
1. "What should I audit?"
2. "How thorough should I be?"
3. "What should I do with confirmed vulnerabilities?"

If flags or clear inline context already provide enough detail, skip the questions and proceed directly.
...
**CI/CD usage:**
```bash
# In GitHub Actions or CI scripts, invoke the same workflow from Codex
...
When `/autoresearch:security` detects a `.github/workflows/` directory, it can offer to generate a security workflow:

Ask in plain text:
"I see you use GitHub Actions. Want me to generate a security audit workflow?"

If yes, create `.github/workflows/security-audit.yml`. If no, continue without CI/CD setup.
    ...
    steps:
      ...
      - name: Install Autoresearch Skill
        run: |
          ...
          cp -r /tmp/autoresearch/skills/autoresearch .codex/skills/autoresearch
          cp -r /tmp/autoresearch/commands/autoresearch .codex/commands/autoresearch
          cp /tmp/autoresearch/commands/autoresearch.md .codex/commands/autoresearch.md
      ...
      - name: Run Security Audit
        ...
        run: |
          ...
          if [ "${{ github.event_name }}" = "pull_request" ]; then
            # invoke the Codex workflow in diff mode
          else
            # invoke the Codex workflow in full-audit mode
          fi
