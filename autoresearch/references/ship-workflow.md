...
**Core idea:** Shipping has a universal pattern regardless of domain. Identify → Checklist → Prepare → Dry-run → Ship → Verify → Log. Treat `/autoresearch:ship` as an alias for this workflow.
...
If `--type` or target is missing, scan for staged changes, open PRs, and recent commits, then ask direct plain-text questions before proceeding. Use scan results to suggest defaults; no special UI tool is required.

Suggested prompts:
1. "What are you shipping?"
2. "How should I ship it?"
3. "Do you want post-ship monitoring?"

If `--type`, `--dry-run`, `--auto`, or `--checklist-only` flags are provided, skip the questions and proceed directly.
