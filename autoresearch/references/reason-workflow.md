...
Structured adversarial refinement for subjective domains. Generate, critique, synthesize, and judge outputs through repeated rounds until convergence, producing a lineage of evolving candidates with documented decision rationale. Simulate the roles locally unless the user explicitly asks for subagents. Use natural-language task framing in Codex; slash-command examples are legacy aliases only.

**Core idea:** Generate-A → Critic attacks A → Generate-B from task+A+critique → Synthesize-AB → Blind judge panel picks winner → winner becomes new A → repeat until convergence. Keep the roles isolated in structure even when one agent performs them locally. Judges should compare randomized candidate labels, not praise a favored author.
...
If task, domain, or mode is missing, ask the user directly in plain text before Phase 1. Batch the clarification into one concise prompt when possible, but do not depend on a question tool.
...
| 5 | `Chain` | "Chain to another workflow after convergence?" | If no chain is provided | "debug", "plan", "fix", "scenario", "predict", "ship", "learn", "No chain — report only" |

**Skip setup entirely when:** Task + Domain + Mode are all provided inline or via flags. Proceed to Phase 1.
...
**STOP: Have you completed the interactive setup above?** If task, domain, or mode is still missing, ask the user directly before entering this phase.
